...
def user_update(request):
    ...
    if datetime.now().month >= 9:
        ...
        user.current_term = 2
     #NEED TO CHANGE THIS TO END YEAR - 5 AND SAVE
    user.end_year = int(request.POST['end_year'])
    user.start_year = user.end_year -5
    user.tuition = Decimal(sanitize_decimal(request.POST['loans']))
    ...
    for item in BudgetPresetData.objects.filter(program=str(user_id)):
        item.delete()
    inserts = []
    labels = request.POST['subjectv']
    amounts= request.POST['amountv']
    incomes= request.POST['budgetType']
    labelssplit    = labels.split(',')
    labelsreal = [','.join(labelssplit[n:]) for n in range(len(labelssplit))]
    amountssplit    = amounts.split(',')
    amountsreal = [','.join(amountssplit[n:]) for n in range(len(amountssplit))]
    rowlength = int(request.POST['rowLength'])
...
#NEED A FOR LOOP IN RANGE FOR NUMBER OF TABLE ROWS IN THE THIRD DIV I THINK
    #FOR EVERY ROW IN IT I WOULD:
    #ADD AN ENTRY INTO BUDGETPRESETDATA:
    #PROGRAM: USER_ID
    #LABEL: THE TEXT VALUE IN THE SUBJECT FIELD
    #YEAR: I DUNNO YET. (CURRENT_YEAR?) MAYB INCLUDE THAT IN THE FOR LOOP
    #INCOME: DDL VALUE OF THE INCOME/EXPENSE
    #
    #INSERTS.APPEND((BUDGETPRESETDATA: BLAHBLAH)
    #BUDGETPRESETDATA.OBJECT.BULK_CREATE(INSERTS)
    for item in BudgetPlanningData.objects.filter(user_id=user_id):
        item.delete()

    initiate_user_preset_data(str(user_id), user) #I WOULD NEED TO CHANGE DEFAULT TO DIFFERENT TYPE SO I DON'T ALTER DEFAULT DATA
